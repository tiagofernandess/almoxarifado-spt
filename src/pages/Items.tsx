...
export default function Items() {
  const { items, addItem, updateItem, deleteItem, loading, reloadData } = useApp();
  ...
  // Filtra os itens com base na busca (Supabase já retorna ordenados por data decrescente)
  const filteredItems = items.filter(item => 
    item.code.toLowerCase().includes(searchQuery.toLowerCase()) ||
    item.name.toLowerCase().includes(searchQuery.toLowerCase())
  );
  
  // Itens que serão exibidos (com paginação)
  const displayedItems = searchQuery 
    ? filteredItems 
    : filteredItems.slice(0, itemsToShow);
  
  // Debug: busca item pelo código
  const findItemByCode = (code: string) => {
    const found = items.find(item => item.code === code);
    console.log(`🔍 Buscando item com código "${code}":`, found);
    console.log('📋 Todos os itens no estado:', items);
    console.log('🔢 Índice do item:', items.findIndex(item => item.code === code));
    alert(found ? `Item encontrado: ${found.name} (ID: ${found.id})\n\nVerifique o Console (F12) para mais detalhes!` : `Item NÃO encontrado com código "${code}"`);
  };
    ...
    <div className="animate-fade-in">
        ...
        <div className="flex flex-wrap gap-2 items-center">
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              setItemsToShow(50); // Reseta a paginação ao buscar
            }}
            className="w-full md:w-80"
          />
          
          {/* Botão de busca por código (debug) */}
          <div className="flex gap-2">
            <Input
              placeholder="Código exato..."
              defaultValue="018523542386"
              className="w-48"
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  findItemByCode(e.currentTarget.value);
                }
              }}
            />
            <Button
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
          <p className="text-blue-700">
            Total de itens no banco: <strong>{items.length}</strong>
          </p>
          <p className="text-blue-700">
              Itens filtrados: <strong>{filteredItems.length}</strong>
          </p>
          <p className="text-blue-700">
              Itens exibidos: <strong>{displayedItems.length}</strong>
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <Table>
          ...
          <TableBody>
            {(() => {
              console.log('🎨 Renderizando tabela com', displayedItems.length, 'itens');
                  ...
                  try {
                    return (
                      <TableRow key={item.id || index}>
                        ...
                        <TableCell className="text-center">{item.totalQuantity ?? 0}</TableCell>
                        <TableCell className="text-center">{item.availableQuantity ?? 0}</TableCell>
                        <TableCell className="text-center">{item.inUseQuantity ?? 0}</TableCell>
                        <TableCell className="text-center">
                          {item.updatedAt ? format(new Date(item.updatedAt), "dd/MM/yy", { locale: ptBR }) : 'N/A'}
                        </TableCell>
                    ...
                    console.error(`❌ Erro ao renderizar item ${index}:`, error, item);
                    return (
                      <TableRow key={index}>
                        <TableCell colSpan={8} className="text-red-500">
                          Erro ao renderizar item: {JSON.stringify(item)}
                        </TableCell>
                      </TableRow>
                    );
                  }
                ...
                return (
                  <TableRow>
                    <TableCell colSpan={8} className="text-center py-6">
                      {searchQuery ? 'Nenhum item encontrado com essa busca' : 'Nenhum item cadastrado'}
                    </TableCell>
                  </TableRow>
                );
              }
        ...
        {/* Botão Ver mais */}
        {!searchQuery && itemsToShow < filteredItems.length && (
          <div className="p-4 border-t flex justify-center">
            ...
            >
              Ver mais {Math.min(50, filteredItems.length - itemsToShow)} itens ({itemsToShow + 50} de {filteredItems.length})
            </Button>
          </div>
        )}
      </div>
