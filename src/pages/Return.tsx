...
export default function Return() {
  ...
  // Filtra os itens para mostrar todos que correspondem à busca
  ...
  const handleAddItemToReturn = () => {
    ...
    if (item.inUseQuantity <= 0) {
      alert('Este item não está em uso e não pode ser devolvido.');
      return;
    }
                    ...
                    <div className="space-y-4 py-4">
                      ...
                      <div className="space-y-2">
                        ...
                        >
                          ...
                          <SelectContent>
                            {inUseItems.map((item) => (
                              <SelectItem key={item.id} value={item.id} disabled={item.inUseQuantity <= 0}>
                                {item.code} - {item.name} {item.inUseQuantity <= 0 ? '(NÃO EM USO)' : `(Em uso: ${item.inUseQuantity})`}
                              </SelectItem>
                            ))}
                          </SelectContent>
                        </Select>
